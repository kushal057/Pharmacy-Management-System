...
$dbh = new Dbh();
$result = $dbh->connect()->prepare("SELECT * FROM medicine WHERE medicine_name = ? AND users_id = ?");
$search =  $_SESSION["old"];
$result->execute(array($search, $_SESSION["users_id"]));
$row = $result->fetch();
...
class Medicine extends Dbh
{
    ...
    function submit()
    {
        ...
        if (!$count > 0) {
            ...
            $search =  $_SESSION["old"];
            $delete->execute(array($search, $this->users_id));
            $_SESSION["old"] = $this->medicine_name;
    ...
    <div class="content">
        ...
        <form action="#" method="post">
            <div class="medicine">
                ...
                <input type="text" placeholder="Medicine Name" name="medicine_name" required value=<?php echo htmlspecialchars($row["medicine_name"])?>>
                <input type="text" placeholder="Medicine Type" name="medicine_type" required  value=<?php echo htmlspecialchars($row["medicine_type"])?>>
            </div>
            <div class="expiry">
            ...
            <div class="amount">
                ...
                <input class="cost_price" type="number" placeholder="Cost Price" name="cost_price" required maxlength="10" value="<?php echo htmlspecialchars($row["cost_price"])?>">
                <input class="marked_price" type="number" placeholder="Marked Price" name="marked_price" required maxlength="10" value="<?php echo htmlspecialchars($row["marked_price"])?>">
                <input class="quantity" type="number" placeholder="Quantity" name="quantity" required maxlength="4" value=<?php echo htmlspecialchars($row["quantity"])?>>
            </div>
            <div class="remarks">
                <label for="name">Notes</label>
                <input type="text" placeholder="Remarks" id="remarks" name="remarks" required maxlength="500" value="<?php echo htmlspecialchars($row["remarks"])?>">
            </div>
